...
describe("ec-cart-item", () => {
  ...
  test("SSR: 商品名・価格・削除ボタン", async () => {
    ...
    const html = renderToString(CI, "ec-cart-item", {
      productId: "tops-001", name: "テスト商品", price: 3900, quantity: 2, image: "/t.webp", stock: 10,
    });
  ...
  test("SSR: 小計 (price * quantity)", async () => {
    const { default: CI } = await import("../../src/components/ec-cart-item.js");
    const html = renderToString(CI, "ec-cart-item", {
      productId: "p-1", name: "商品", price: 5000, quantity: 3, image: "/i.webp", stock: 10,
    });
  ...
  test("SSR: ec-quantity-selector 含む", async () => {
    const { default: CI } = await import("../../src/components/ec-cart-item.js");
    const html = renderToString(CI, "ec-cart-item", {
      productId: "p-1", name: "商品", price: 1000, quantity: 1, image: "/i.webp", stock: 10,
    });
  ...
  test("DOM: 削除ボタン存在", async () => {
    ...
    el.setAttribute("product-id", "tops-001"); el.setAttribute("name", "テスト");
    el.setAttribute("price", "3900"); el.setAttribute("quantity", "1");
    el.setAttribute("image", "/t.webp"); el.setAttribute("stock", "10");
