...
function getSessionId(): string {
  ...
  if (!id) { id = crypto.randomUUID(); localStorage.setItem("ec-session-id", id); }
  return id;
}
...
export async function addToCart(productId: string, quantity: number): Promise<void> {
  await fetch(`${BASE}/cart/items`, { method: "POST", headers: headers(), body: JSON.stringify({ productId, quantity }) });
}
export async function updateCartItem(productId: string, quantity: number): Promise<void> {
  await fetch(`${BASE}/cart/items/${productId}`, { method: "PUT", headers: headers(), body: JSON.stringify({ quantity }) });
}
...
export async function createOrder(customer: { name: string; email: string; address: string }): Promise<Order> {
  const res = await fetch(`${BASE}/orders`, { method: "POST", headers: headers(), body: JSON.stringify(customer) });
  return res.json();
}
