...
:host { @apply block; }`,
  ...
  (props) => {
    ...
    const dispatch = useEvent<{ productId: string; quantity: number }>("cart-add", { bubbles: true, composed: true });
    const host = useHost();
    ...
    const btnBg = added ? "bg-accent-success text-white" : "bg-accent text-white hover:bg-accent-hover";
    return `
