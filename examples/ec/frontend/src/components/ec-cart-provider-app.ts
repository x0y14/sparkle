import { defineElement, useState, useEffect, useCallback, useHost } from "@sparkle/core";
import { CartContext, type CartContextValue } from "../context/cart-context.js";
import type { CartItem } from "../lib/types.js";

const CartProviderApp = defineElement(
  { tag: "ec-cart-provider-app" },
  () => {
    ...
    const addItem = useCallback(async (productId: string, quantity: number) => {
      ...
      await addToCart(productId, quantity); await refresh();
    }, []);

    const removeItem = useCallback(async (productId: string) => {
      const { removeCartItem } = await import("../lib/api.js");
      await removeCartItem(productId); await refresh();
    }, []);

    const updateQuantity = useCallback(async (productId: string, quantity: number) => {
      const { updateCartItem } = await import("../lib/api.js");
      await updateCartItem(productId, quantity); await refresh();
    }, []);

    const totalCount = items.reduce((s, i) => s + i.quantity, 0);
    const totalPrice = items.reduce((s, i) => s + (i.product?.price ?? 0) * i.quantity, 0);
    const ctx: CartContextValue = { items, totalCount, totalPrice, addItem, removeItem, updateQuantity, refresh };
