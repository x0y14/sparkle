import { defineElement, useState, useContext, useHost, useEffect, css } from "@sparkle/core";
  ...
  () => {
    ...
    useEffect(() => {
      ...
      const clickHandler = async (e: Event) => {
        ...
        } catch { setErrors(["注文の処理中にエラーが発生しました"]); setSubmitting(false); }
      };
      root.addEventListener("field-change", fieldHandler);
      root.addEventListener("click", clickHandler);
      return () => { root.removeEventListener("field-change", fieldHandler); root.removeEventListener("click", clickHandler); };
