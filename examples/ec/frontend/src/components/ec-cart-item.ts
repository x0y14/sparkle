...
const CartItemEl = defineElement(
  {
    ...
    props: {
      ...
      price: { type: Number, value: () => 0 }, quantity: { type: Number, value: () => 1 },
      image: { type: String }, stock: { type: Number, value: () => 99 },
    },
...
:host { @apply block; }`,
  ...
  (props) => {
    ...
    useEffect(() => {
      ...
      const qtyHandler = (e: Event) => cart.updateQuantity(props.productId, (e as CustomEvent<number>).detail);
      root.addEventListener("click", clickHandler);
      root.addEventListener("quantity-change", qtyHandler);
      return () => { root.removeEventListener("click", clickHandler); root.removeEventListener("quantity-change", qtyHandler); };
    }, [props.productId]);
