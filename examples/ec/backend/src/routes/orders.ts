...
app.post("/", async (c) => {
  const { name, email, address } = await c.req.json<{ name: string; email: string; address: string }>();
  if (!name || !email || !address) return c.json({ error: "name, email, address are required" }, 400);
  ...
  const order: Order = {
    id: `order-${Date.now()}`, items: orderItems, total,
    customer: { name, email, address }, createdAt: new Date().toISOString(),
  };
