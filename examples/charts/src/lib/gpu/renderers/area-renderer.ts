...
import areaShader from "../shaders/area.wgsl?raw"

export class AreaRenderer implements ChartRenderer {
  ...
  prepare(gpu: GPUContext, series: NormalizedSeries[], uniforms: RenderUniforms): void {
    ...
    if (!this.pipeline) {
      const shaderModule = gpu.device.createShaderModule({ code: areaShader })
