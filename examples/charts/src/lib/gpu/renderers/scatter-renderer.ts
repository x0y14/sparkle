...
export class ScatterRenderer implements ChartRenderer {
  ...
  prepare(gpu: GPUContext, series: NormalizedSeries[], uniforms: RenderUniforms): void {
    ...
    // Pack as PointInstance: position(vec2) + radius(f32) + pad(f32) = 4 floats per point
    const pointData = new Float32Array(this.pointCount * 4)
    for (let i = 0; i < this.pointCount; i++) {
      pointData[i * 4 + 0] = s.accessor.getX(i) - s.offsetX
      pointData[i * 4 + 1] = s.accessor.getY(i) - s.offsetY
      pointData[i * 4 + 2] = s.pointRadius
      pointData[i * 4 + 3] = 0 // padding
    }
    this.dataBuffer.write(pointData)
    ...
    if (!this.pipeline) {
      const shaderModule = gpu.device.createShaderModule({ code: scatterShader })
