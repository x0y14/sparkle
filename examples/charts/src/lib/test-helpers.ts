...
import type { GPUContext, ChartConfig, ChartType, NormalizedSeries, PointObject, OHLCObject } from "./types"
...
export function makeNormalizedSeries(
  configs: { data: PointObject[]; label: string; stackId?: string }[],
  type: ChartType,
): NormalizedSeries[] {
  const fullConfig: ChartConfig = { type, series: configs }
  return normalizeConfig(fullConfig).series
}

export function makeNormalizedPieSeries(
  values: number[],
  innerRadius: number = 0,
): NormalizedSeries[] {
  const configs = values.map((v, i) => ({
    data: [{ x: 0, y: v }] as PointObject[],
    label: `S${i}`,
    innerRadius,
  }))
  return normalizeConfig({ type: "pie" as ChartType, series: configs }).series
}

export function makeNormalizedOHLCSeries(
  data: OHLCObject[],
): NormalizedSeries {
