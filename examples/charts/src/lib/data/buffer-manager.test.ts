...
describe("BufferManager", () => {
  ...
  it("reallocate 時に旧バッファの destroy が呼ばれる", () => {
    ...
    const mgr = new BufferManager(device, GPUBufferUsage.VERTEX)
    mgr.write(new Float32Array([1])) // 4 bytes → cap=4
    const oldDestroyCalls = destroySpy.mock.calls.length
    mgr.write(new Float32Array([1, 2, 3, 4, 5])) // 20 bytes → realloc
  ...
  it("append がデータを末尾に追加し usedBytes を累積する", () => {
    const { device } = createMockGPUDevice()
    const mgr = new BufferManager(device, GPUBufferUsage.VERTEX)
    mgr.write(new Float32Array([1, 2])) // 8 bytes
    mgr.append(new Float32Array([3, 4])) // 8 bytes
  ...
  it("append 中の realloc で usedBytes が正しく累積される", () => {
    ...
    const mgr = new BufferManager(device, GPUBufferUsage.VERTEX)
    mgr.write(new Float32Array([1, 2, 3, 4])) // 16 bytes, cap=16
    mgr.append(new Float32Array([5, 6, 7, 8])) // +16 bytes → realloc
  ...
  it("append が容量超過時に realloc する", () => {
    ...
    const mgr = new BufferManager(device, GPUBufferUsage.VERTEX)
    mgr.write(new Float32Array([1])) // 4 bytes, cap=4
    mgr.append(new Float32Array([2, 3])) // +8 bytes → realloc
