...
:host { @apply block; }`,
  ...
  (props) => {
    ...
    return `<div class="${variantClass} rounded-lg p-2 transition-colors ${invalidClass} ${disabledClass}">
  <textarea rows="${props.rows}" placeholder="${props.placeholder}" class="w-full bg-transparent outline-none text-foreground placeholder-default-400 text-sm resize-y" ${props.isDisabled ? "disabled" : ""} ${props.isReadOnly ? "readonly" : ""} ${props.isInvalid ? 'aria-invalid="true"' : ""}>${value}</textarea>
</div>`
  },
)
