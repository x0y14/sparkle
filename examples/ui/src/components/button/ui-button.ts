...
:host([full-width]) { @apply block; }`,
  ...
  (props) => {
    ...
    const sizeClass = props.isIconOnly
      ? (V.buttonIconOnlySize[props.size as V.Size] || V.buttonIconOnlySize.md)
      : (V.buttonSize[props.size as V.Size] || V.buttonSize.md)
    ...
    return `<button class="inline-flex items-center justify-center font-medium transition-all ${radiusClass} ${sizeClass} ${colorClass} ${widthClass} ${disabledClass}" ${props.isDisabled ? "disabled" : ""} ${props.isLoading ? 'aria-busy="true"' : ""}>${spinner}<slot></slot></button>`
  },
)
