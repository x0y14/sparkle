...
import { sparkleIntegration } from "../src/index.js"

describe("sparkleIntegration", () => {
  test("returns valid AstroIntegration", () => {
    const integration = sparkleIntegration()
  ...
  test("name is '@sparkle/astro'", () => {
    const integration = sparkleIntegration()
    expect(integration.name).toBe("@sparkle/astro")
  })
  ...
  test("addRenderer called with correct entrypoints", () => {
    ...
    expect(rendererConfig.name).toBe("@sparkle/astro")
  ...
  test("polyfill:false skips injection", () => {
    const integration = sparkleIntegration({ polyfill: false })
    const addRenderer = vi.fn()
    const updateConfig = vi.fn()
    const injectScript = vi.fn()
  ...
  test("unoConfig が sparkleVitePlugin に渡される", () => {
    const fakeUnoConfig = { presets: [] }
    const integration = sparkleIntegration({ unoConfig: fakeUnoConfig })
    const addRenderer = vi.fn()
    const updateConfig = vi.fn()
    const injectScript = vi.fn()
