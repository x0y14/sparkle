...
import { blaskVitePlugin } from "../src/plugin.js"

describe("blaskVitePlugin", () => {
  test("plugin name is 'blask:vite'", () => {
  ...
  test("transform: lazy-inits uno if buildStart not called", async () => {
    const plugin = blaskVitePlugin({ unoConfig: { presets: [] } })
    const transform = plugin.transform as Function
    const result = await transform("const css = `@unocss-placeholder`;", "test.ts")
    if (result) {
      expect(result.code).not.toContain("@unocss-placeholder")
  ...
  test("resolveId: recognizes virtual:blask/uno.css", () => {
    const plugin = blaskVitePlugin()
    const resolveId = plugin.resolveId as Function
    const result = resolveId("virtual:blask/uno.css")
    expect(result).toBe("\0virtual:blask/uno.css")
  })
  ...
  test("load: virtual module returns CSSStyleSheet with replaceSync", async () => {
    ...
    const result = await load("\0virtual:blask/uno.css")
