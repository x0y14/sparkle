...
describe("morph", () => {
  ...
  test("id elements matched and state preserved across moves", () => {
    const root = createRoot('<div id="x" class="old">X</div><div id="y">Y</div>')
  ...
  test("duplicate IDs excluded from persistent matching", () => {
    const root = createRoot('<div id="dup">A</div><div id="dup">B</div>')
  ...
  test("nested custom elements: child shadowRoot untouched", () => {
    ...
    expect(root.querySelector("div")!.shadowRoot!.innerHTML).toBe("<span>shadow content</span>")
  })
  ...
  test("context-dependent tags like <tr> parse correctly", () => {
    ...
    morph(root, "<table><tbody><tr><td>B</td></tr></tbody></table>")
  ...
  test("deeply nested update only changes leaf", () => {
    const root = createRoot("<div><section><p><span>old</span></p></section></div>")
    const span = root.querySelector("span")!
    morph(root, "<div><section><p><span>new</span></p></section></div>")
