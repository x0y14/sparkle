...
describe("defineElement", () => {
  ...
  test("multiple instances share CSSStyleSheet", async () => {
    ...
    expect(el1.shadowRoot!.adoptedStyleSheets[0]).toBe(el2.shadowRoot!.adoptedStyleSheets[0])
  ...
  test("Boolean coercion: '0' is false", async () => {
    ...
    defineElement({ tag, props: { active: { type: Boolean } } }, (props: any) => {
  ...
  test("Number coercion: '' is invalid (keeps default)", async () => {
    const tag = uniqueTag()
    let captured: unknown
    defineElement({ tag, props: { count: { type: Number, value: () => 42 } } }, (props: any) => {
