// Scratch morph implementation for sparkio
...
function populateIdMap(
  ...
  for (const el of elements) {
    ...
    while (current) {
      ...
      current = (current as Element).parentElement ?? (current as any).parentNode
    }
  }
}
...
function morphChildren(
  ...
  if (actualOld instanceof HTMLTemplateElement && actualNew instanceof HTMLTemplateElement) {
  ...
  for (const newChild of Array.from(actualNew.childNodes)) {
    if (insertionPoint) {
      const bestMatch = findBestMatch(newChild, insertionPoint, null, idMap, activeElements)
    ...
    if (newChild instanceof Element) {
      ...
      if (newChildId && persistentIds.has(newChildId)) {
        const found = (actualOld as Element).querySelector?.(`[id="${CSS.escape(newChildId)}"]`)
        if (found) {
...
function syncInputValue(oldElt: Element, newElt: Element): void {
  ...
  if (oldElt instanceof HTMLSelectElement && newElt instanceof HTMLSelectElement) {
...
function restoreFocusState(root: Element | ShadowRoot, saved: SavedFocus): void {
  if (!saved) return
  ...
  if (saved.id) {
    ...
    if (saved.id === currentActive?.getAttribute("id")) {
      ...
      target = root.querySelector(`[id="${CSS.escape(saved.id)}"]`)
    }
  } else if (saved.path) {
  ...
  if (
    ...
    if (
      active.selectionStart !== saved.selectionStart ||
