...
export type SparkioComponentOptions<P extends Record<string, PropType>> = {
  props?: P
...
export type SparkioRenderFn<P> = (props: P) => string | Node | null
...
export function coerceValue(value: string | null, type: Function): CoerceResult {
    ...
    case Object:
      try {
        ...
        if (typeof console !== "undefined") {
          console.warn(`[sparkio] Failed to parse attribute value as JSON: "${value}"`)
        }
        return { ok: false }
      }
    default:
...
export function defineElement<P extends Record<string, PropType>>(
  ...
  // Astro の SSR renderer は _renderFn / _tag / __sparkio のみ使うためスタブを返す
  if (typeof HTMLElement === "undefined") {
    return Object.assign(class SparkioSSRComponent {}, {
      __sparkio: true,
      _renderFn: renderFn,
      _tag: options.tag,
  ...
  class SparkioElement extends HTMLElement {
    ...
    constructor() {
      ...
      if (options.shadow !== false) {
        ...
        if (!hasDSD) {
          const init =
            typeof options.shadow === "object" ? options.shadow : ({ mode: "open" } as const)
    ...
    _render() {
      ...
      try {
        const html = runInContext(this._hookCtx, () => renderFn(this._props as InferProps<P>))
        ...
        console.error(`[sparkio] Render error in <${options.tag || "unknown"}>:`, err)
  ...
  for (const [name, schema] of Object.entries(propsSchema)) {
    Object.defineProperty(SparkioElement.prototype, name, {
      get(this: SparkioElement) {
        return this._props[name]
      },
      set(this: SparkioElement, value: unknown) {
        ...
        if (schema.event && !Object.is(oldValue, value)) {
          this.dispatchEvent(
            new CustomEvent(schema.event.type, {
              detail: value,
  ...
  if (options.tag && typeof customElements !== "undefined") {
    customElements.define(options.tag, SparkioElement)
  }

  return SparkioElement
}
